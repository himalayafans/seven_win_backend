...
namespace SevenWinBackend.Application.Games.SevenWin.Strategies
{
    ...
    internal class GameAdditionStrategy : BaseStrategy
    {
        ...
        public override async Task Handle(StrategyContext context)
        {
            ...
            if (baseRecordView.ImageId != context.Cache.ImageId)
            {
            ...
            {
                var game = await gameService.GetPlayerGameById(context.Cache.PlayerGameId);
                var scoreDetail = game!.GetScoreDetail<SevenWinScoreDetail>();
                var emptyChannelId = scoreDetail.EmptyChannelId;
                var channel = await channelService.GetById(emptyChannelId);
                if (channel == null)
                {
                    // 不可能为空
                    throw new InvalidOperationException();
                ...
                else
                {
                    ...
                    context.PlayResult.AddMessage($"恭喜赢得附加赛，获得{score}个玉米,空房间为:{channel.Name}");
                }
            }
            else
            {
